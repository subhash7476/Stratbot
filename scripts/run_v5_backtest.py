"""
PixityAI v5 — Clean Momentum Baseline
======================================
...
PIPELINE:
  ...
  3. Entry: T+1 open
  4. Trade state machine: 1.5xATR stop, trail after +1.5R at 1xATR, time-stop 10 days
  5. Portfolio: 0.75% risk, max 5 concurrent, 5-day re-entry guard per symbol

SPEC (locked):
  Stop:      1.5 × ATR(20) from signal day close
  Target:    NONE — trail only
  Trail:     after +1.5R → trail at 1×ATR below price (long), updated each close
  Time stop: 10 trading days
  No early exit. No fixed target. Let momentum breathe.
...
Usage:
    python scripts/run_v5_backtest.py
    python scripts/run_v5_backtest.py --debug-trades
    python scripts/run_v5_backtest.py --universe-size 60
"""
...
class Trade:
    """
    v5 Long-only trade lifecycle on daily bars.

    Spec:
      Stop:      entry - 1.5*ATR
      Target:    NONE (trail only)
      Trail:     after +1.5R → trail stop at 1*ATR below close each day
      Time stop: 10 trading days
      No early exit rule.
    """
    ...
    TRAIL_R        = 1.5     # activate trail after +1.5R
    TRAIL_ATR_MULT = 1.0     # trail at 1× ATR below close
    ...
    def update_bar(self, bar: pd.Series) -> Optional[str]:
        ...
        Order of operations:
          ...
          2. Update trail using bar close
          3. Time stop at end of bar
        """
        if self.closed:
            return None
...
def print_report(portfolio: Portfolio, label: str, start: date, end: date) -> dict:
    ...
    print(f"    Time-stop dominated: {'YES [watch]' if exits.get('TIME_STOP', 0) > len(trades) * 0.6 else 'no'}")
    print(f"{'='*65}\n")
...
def main():
    parser = argparse.ArgumentParser(description="PixityAI v5 Clean Momentum Backtest")
    ...
    for k in ["trades", "win_rate", "profit_factor", "avg_r", "median_r",
              "skew", "stops", "time_stops", "force_closes", "max_dd", "total_pnl"]:
