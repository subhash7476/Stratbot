"""
Research Script: Enhanced Stock Day-Type Classifier at 10:00 AM
============================================================
Tests if adding Relative Strength, VWAP proximity, and Volume Expansion 
improves accuracy at the 10:00 AM (45-min) checkpoint.

Features:
  - e_ret, e_range, e_close_loc (Original)
  - rel_strength (vs Nifty 50)
  - dist_vwap (Normalized distance from VWAP)
  - vol_surge (Current volume vs session average)

Output: models/daytype/stock_1000am_enhanced/
"""
...
MODEL_DIR  = ROOT / "models" / "daytype" / "stock_1000am_enhanced"

CHECKPOINT_BARS = 45          # 9:15 to 10:00 (45 bars)
FEATURE_NAMES   = ["e_ret", "e_range", "e_close_loc", "rel_strength", "dist_vwap", "vol_surge"]
...
def compute_enhanced_features(bars: pd.DataFrame, nf_bars: pd.DataFrame) -> dict | None:
    ...
    # 4. Volume Surge (Relative to its own early session)
    # Using first 15m as baseline for the 45m surge
...
def extract_data(symbols: list[str]):
    ...
    # Speed up research: focus on 2025 and 2026
...
def train_and_eval(train_thru=2025):
    ...
    # Extreme memory optimization: only load Top 20 symbols for research validation
    labels = pd.read_csv(LABEL_CSV, usecols=["date", "symbol", "cluster", "day_type"])
    top_symbols = labels["symbol"].value_counts().head(20).index.tolist()
    labels = labels[labels["symbol"].isin(top_symbols)]
    labels["date"] = pd.to_datetime(labels["date"])
    symbols = labels["symbol"].unique().tolist()
    
    print(f"Extracting enhanced 10:00 AM features for TOP 20 symbols ({len(symbols)} total)...")
