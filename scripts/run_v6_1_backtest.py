"""
PixityAI v6.1 — Momentum + Nifty 200DMA Regime Filter
======================================
...
PIPELINE:
  ...
  3. Entry: T+1 open
  4. Regime filter: only enter new longs when Nifty 50 daily close > 200-day SMA. Cash otherwise.
  5. Trade state machine: 1.5xATR initial stop, 2xATR trailing stop on highest close. No time cap.
...
SPEC (locked):
  ...
  Regime gate: Nifty 200DMA (structural, not tuned). Single shot — no optimization.
...
Usage:
    python scripts/run_v6_1_backtest.py
    python scripts/run_v6_1_backtest.py --debug-trades
    python scripts/run_v6_1_backtest.py --universe-size 60
"""
...
def run_backtest(
    ...
    # Pre-compute Nifty 200DMA gate for each trading day
    # nifty_gate[td] = True means Nifty close >= 200DMA -> new entries allowed
    nifty_gate: Dict[date, bool] = {}
    if nifty_df is not None and not nifty_df.empty:
        nifty_indexed = nifty_df.set_index("date")["close"]
        nifty_sma200  = nifty_indexed.rolling(200, min_periods=100).mean()
        for td in trading_days:
            if td in nifty_indexed.index and td in nifty_sma200.index:
                c = float(nifty_indexed.loc[td])
                m = float(nifty_sma200.loc[td])
                nifty_gate[td] = (not np.isnan(m)) and (c >= m)
            else:
                nifty_gate[td] = True   # no data -> allow
        n_active = sum(nifty_gate.values())
        logger.info(f"Nifty 200DMA gate: {n_active}/{len(trading_days)} entry days active")
    else:
        nifty_gate = {td: True for td in trading_days}
        logger.info("Nifty 200DMA gate: DISABLED (no Nifty data)")
    ...
    for td in trading_days:
        ...
        # Regime gate: skip new entries on days when Nifty < 200DMA
        regime_ok = nifty_gate.get(td, True)
        sigs_today = signals_by_entry.get(td, []) if regime_ok else []
...
def main():
    parser = argparse.ArgumentParser(description="PixityAI v6.1 Momentum + 200DMA Regime Filter")
    ...
    # Universe selection — rank as of the last trading day with available data
    # that is <= train_start. We use the last available day in preloaded cache
    # up to (but not including) train_start to avoid look-ahead.
    # Find last trading day strictly before tr_s
    all_preload_days = get_trading_days(ROOT / "data", preload_from, tr_s - timedelta(days=1))
    if all_preload_days:
        rank_as_of = all_preload_days[-1]
    else:
        rank_as_of = tr_s
    logger.info(f"Selecting universe as of {rank_as_of}...")
    ...
    # Load Nifty daily for 200DMA regime gate
    # Need data from preload_from so the 200DMA has enough history by train start
    logger.info("Loading Nifty daily for 200DMA gate...")
    nifty_df = preload_nifty(ROOT / "data", preload_from, te_e)
    if nifty_df is not None:
        logger.info(f"Nifty data: {len(nifty_df)} days "
                    f"({nifty_df['date'].min()} to {nifty_df['date'].max()})")
    else:
        logger.warning("No Nifty data found — 200DMA gate will be disabled")
